...
namespace accd {
        ...
        const Var<float> offset)
    {
        ...
        if constexpr (print_ccd_iter_count)
            $if(iter_count != 1) { LUISA_INFO("CCD iter for {}, toi = {}", iter_count, toi); };
...
namespace host_accd {
    ...
    struct EdgeEdgeSquaredDist {
        inline float operator()(const Mat3x4f& x)
        {
            ...
            return host_distance::edge_edge_distance_squared_unclassified(p0, p1, q0,
                q1);
        }
    };
    ...
    struct PointTriangleSquaredDist {
        inline float operator()(const Mat3x4f& x)
        {
            ...
            return host_distance::point_triangle_distance_squared_unclassified(p, t0, t1,
                t2);
        }
    };
    ...
    inline float point_triangle_ccd(const Vec3f& p0, const Vec3f& p1,
        const Vec3f& t00, const Vec3f& t01,
        const Vec3f& t02, const Vec3f& t10,
        const Vec3f& t11, const Vec3f& t12,
        float offset)
    ...
    inline float edge_edge_ccd(const Vec3f& ea00, const Vec3f& ea01,
        const Vec3f& eb00, const Vec3f& eb01,
        const Vec3f& ea10, const Vec3f& ea11,
        const Vec3f& eb10, const Vec3f& eb11,
        float offset)
