...
namespace host_distance {
    ...
    {
        Vec3f c = point_triangle_distance_coeff_unclassified(p, t0, t1, t2);
        Vec3f x = c(0) * (t0 - p).template cast<float>() + c(1) * (t1 - p).template cast<float>() + c(2) * (t2 - p).template cast<float>();
        ...
        const SVec<T, 3>& eb1)
    {
        ...
        if (c[0] >= 0.0f && c[0] <= 1.0f && c[1] >= 0.0f && c[1] <= 1.0f && c[2] >= 0.0f && c[2] <= 1.0f && c[3] >= 0.0f && c[3] <= 1.0f) {
            return c;
        } else {
            Vec2f c1 = point_edge_distance_coeff(ea0, eb0, eb1);
            Vec2f c2 = point_edge_distance_coeff(ea1, eb0, eb1);
