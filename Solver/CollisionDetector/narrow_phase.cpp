...
void NarrowPhasesDetector::download_broadphase_collision_count(Stream &stream) {
    ...
    if (num_vf_broadphase > collision_data->broad_phase_list_vf.size() / 2) { LUISA_ERROR("BroadPhase VF outof range : {} ({})", num_vf_broadphase, collision_data->broad_phase_list_vf.size() / 2); }
    if (num_ee_broadphase > collision_data->broad_phase_list_ee.size() / 2) { LUISA_ERROR("BroadPhase EE outof range : {} ({})", num_ee_broadphase, collision_data->broad_phase_list_ee.size() / 2); }
        ...
        [broadphase_count = collision_data->broad_phase_collision_count.view(offset_vf, 1),
         ...
         narrowphase_list_vf = collision_data->narrow_phase_list_vf.view(), contact_energy_type](
            ...
            $if (
                vid == face[0] |
                vid == face[1] |
                vid == face[2] | (vert_is_rigid_body(sa_vert_affine_bodies_id_left.read(vid)) | vert_is_rigid_body(sa_vert_affine_bodies_id_right.read(face[0])))) {
            }
            $else {
