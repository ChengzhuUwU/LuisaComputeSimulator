...
namespace BendingEnergy
{
    ...
    {
        if (Denominator <= FLOAT_SMALL_NUMBER)
    ...
    inline float CalcGradientsAndAngle(const luisa::float3& P1,
                                       const luisa::float3& P2,
                                       const luisa::float3& P3,
                                       const luisa::float3& P4,
                                       luisa::float3&       Grad1,
                                       luisa::float3&       Grad2,
                                       luisa::float3&       Grad3,
                                       luisa::float3&       Grad4)
    {
        ...
        const FVector3f DPhiDN1_OverNormal1Len =
            SafeDivide(CosPhi * luisa::cross(SharedEdgeNormalized, Normal2) - SinPhi * Normal2, Normal1Len);
        const FVector3f DPhiDN2_OverNormal2Len =
            SafeDivide(CosPhi * luisa::cross(Normal1, SharedEdgeNormalized) - SinPhi * Normal1, Normal2Len);
