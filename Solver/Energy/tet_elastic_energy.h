...
{
    float I3 = a * b * c;
    float coeff = lmd * (I3 - 1.0f) - mu;

    // Diagonal:  d2Psi/da^2 = mu + lmd*(bc)^2
    d2[0][0] = mu + lmd * (b * c) * (b * c);
    d2[1][1] = mu + lmd * (a * c) * (a * c);
    d2[2][2] = mu + lmd * (a * b) * (a * b);

    // Off-diagonal:  d2Psi/(da*db) = lmd*(bc)*(ac) + coeff*c
    //   = lmd*I3*c + (lmd*(I3-1)-mu)*c  -- simplify:
    //   = c * (lmd*I3 + lmd*I3 - lmd - mu) ... let's keep explicit
    //   d2Psi/(da*db) = dcoeff/db * (b*c) + coeff * c
    //   dcoeff/db = lmd * a*c
    //   = lmd*a*c*(b*c) + coeff*c = lmd*I3*c + coeff*c
    d2[0][1] = d2[1][0] = lmd * I3 * c + coeff * c;  // = c*(lmd*I3 + coeff)
    ...
    float eps = 1e-8f)
{
    ...
    {
        float D[3][9];
        for (int k = 0; k < 3; k++) make_stretch_mode(k, U, V, D[k]);
        ...
        for (int iter = 0; iter < 20; iter++) {
            ...
            if (std::abs(M[0][2]) > maxval) { maxval = std::abs(M[0][2]); pi = 0; pj = 2; }
            if (std::abs(M[1][2]) > maxval) { maxval = std::abs(M[1][2]); pi = 1; pj = 2; }
            if (maxval < 1e-12f) break;
...
// Result stored as luisa float3x3 (col-major: M[col][row])
    ...
    luisa::float3x3 hessian[16])
{
    ...
    // 5. Map Hessian to F-space (9x9, PD-projected)
    lcs::float9x9 d2edF2 = eigenanalysis_hessian(U, V, S, deda, d2);
        ...
        for (int beta = 0; beta < 4; beta++)
        {
            ...
            for (int j = 0; j < 3; j++)     // col = beta spatial dim
                for (int i = 0; i < 3; i++) // row = alpha spatial dim
                {
                    ...
                    if (i == 0) {
                        if      (j == 1) cross_term =  Fc2;
                        else if (j == 2) cross_term = -Fc1;
                    } else if (i == 1) {
                        if      (j == 0) cross_term = -Fc2;
                        else if (j == 2) cross_term =  Fc0;
                    } else {  // i == 2
                        if      (j == 0) cross_term =  Fc1;
                        else if (j == 1) cross_term = -Fc0;
                    }
