    ...
    std::vector<uint>& inner_tets, std::vector<uint>& outer_tets, std::vector<Int3>& surface_faces, std::vector<uint>& surface_verts)
{
    ...
    CpuParallel::parallel_for(0, num_tets, [&](uint tid)
    {
    ...
    CpuParallel::parallel_for(0, tmp_tets.size(), [&](const uint i)
    {
    ...
    CpuParallel::parallel_for_and_scan(0, tmp_tets.size(), [&](const uint i)
    {
    ...
    uint num_surface_verts = CpuParallel::parallel_for_and_reduce_sum<uint>(0, num_verts, [&](const uint vid) { return list_vert_is_on_surface[vid] ? 1 : 0; });
    surface_verts.resize(num_surface_verts);
    CpuParallel::parallel_for_and_scan(0, num_verts, [&](const uint vid)
    {
    ...
    CpuParallel::single_thread_for(0, num_tets, [&](uint tid)
    {
        Int4 tet = input_tets[tid];
    ...
    std::vector<Int2>& output_edges, std::vector<Int4>& output_bending_edges)
{
    ...
    CpuParallel::parallel_for(0, num_surface_faces, [&](const uint fid)
    {
    ...
    CpuParallel::parallel_sort(tmp_faces.begin(), tmp_faces.end(), [](const Int3& left, const Int3& right)
    {
        int temp;
        temp = left[0] - right[0]; if(temp != 0) return temp < 0;
        temp = left[1] - right[1]; if(temp != 0) return temp < 0;
    ...
    CpuParallel::parallel_for(0, tmp_faces.size(), [&](const uint i)
    {
    ...
    CpuParallel::parallel_for_and_scan(0, tmp_faces.size(), [&](const uint i)
    {
        // edge_type:
        //      0 : Boundary
    ...
    if (extract_bending_edge)
    {
        ...
        [&](const uint i, const uint& prefix, const uint& curr_return)
        {
            if (curr_return == 1)
            {
                ...
                output_bending_edges[eid] = Int4{dehedral_edge[0], dehedral_edge[1], curr_rest_vid, next_rest_vid};
            }
        }, 0u);
    }
}
