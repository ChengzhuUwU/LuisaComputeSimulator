...
namespace CollisionPair {
    ...
    template <typename T>
    auto get_vv_vid1(const T& pair) { return pair.indices[0]; }
    template <typename T>
    auto get_vv_vid2(const T& pair) { return pair.indices[1]; }
    template <typename T>
    auto get_ve_vid(const T& pair) { return pair.vid; }
    template <typename T>
    auto get_ve_edge(const T& pair) { return pair.edge; }
    template <typename T>
    auto get_vf_vid(const T& pair) { return pair.indices[0]; }
    template <typename T>
    auto get_vf_face(const T& pair) { return pair.indices.yzw(); }
    template <typename T>
    auto get_ee_edge1(const T& pair) { return pair.indices.xy(); }
    template <typename T>
    auto get_ee_edge2(const T& pair) { return pair.indices.zw(); }

    // template<typename T> auto get_stiff(const T& pair) { return pair.vec1[3]; }
    template <typename T>
    auto get_direction(const T& pair) { return pair.vec1.xyz(); }
    template <typename T>
    auto get_area(const T& pair) { return pair.vec1[3]; }
    ...
    template <typename T>
    auto get_vf_stiff(const T& pair) { return pair.vec2; }
    template <typename T>
    auto get_ee_stiff(const T& pair) { return pair.vec2; }

    template <typename T>
    auto get_vf_k1(const T& pair) { return pair.vec2[0]; }
    template <typename T>
    auto get_vf_k2(const T& pair) { return pair.vec2[1]; }
    template <typename T>
    auto get_ee_k1(const T& pair) { return pair.vec2[0]; }
    template <typename T>
    auto get_ee_k2(const T& pair) { return pair.vec2[1]; }
...
struct CollisionData : SimulationType {
        ...
        const uint num_verts, const uint num_faces, const uint num_edges)
    {
        ...
        const uint collision_pair_bytes = sizeof(uint) * this->broad_phase_list_vf.size() + sizeof(uint) * this->broad_phase_list_ee.size() + sizeof(CollisionPairVV) * this->narrow_phase_list_vv.size() + sizeof(CollisionPairVE) * this->narrow_phase_list_ve.size() + sizeof(CollisionPairVF) * this->narrow_phase_list_vf.size() + sizeof(CollisionPairEE) * this->narrow_phase_list_ee.size();
        ...
        if (float(collision_pair_bytes) / (1024 * 1024 * 1024) > 1.0f)
            LUISA_INFO("Allocated buffer size for collision pair = {} GB", collision_pair_bytes / (1024 * 1024 * 1024));
    }
};
