...
double SolverInterface::host_compute_elastic_energy(const std::vector<float3>& curr_x)
{
    auto compute_energy_inertia = [](
        ...
        if constexpr (print_detail)
            LUISA_INFO("    vid {} inertia energy {} (|dx| = {})", vid, energy, sqrt_scalar(length_squared_vec(x_new - x_tilde)));
        return energy;
    };
    ...
    fn_calc_energy_inertia = device.compile<1>(
        [sa_x_tilde = sim_data->sa_x_tilde.view(),
...

            {
                ...
                $else {};
                if constexpr (print_detail)
                    device_log("vid {}, inertia energy {} (invdt2 = {}, |dx|2 = {}, diff = {}) mass = {}", vid, energy, squared_inv_dt, (length_squared_vec(x_new - x_tilde)), x_new - x_tilde, mass);
            };
