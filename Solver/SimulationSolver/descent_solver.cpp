...
[[nodiscard]] auto operator*(const luisa::compute::Var<TestTypeAAA>& left,
                             const luisa::compute::Var<TestTypeAAA>& right) noexcept
{
...
void DescentSolver::compile(luisa::compute::Device& device)
{
    ...
    fn_evaluate_inertia = device.compile<1>(
        [sa_Hf        = sim_data->sa_Hf.view(),
         ...
         writeHf,
         makeHf1](const Float substep_dt)
        {
            const UInt   vid = dispatch_id().x;
            const Float3 gravity(0, -9.8f, 0);
    ...
    { return make_float3x3(left * right[0], left * right[1], left * right[2]); };
    ...
    fn_evaluate_stretch_spring = device.compile<1>(
        [sa_Hf                 = sim_data->sa_Hf.view(),
         ...
         writeHf,
         outer_product,
         makeHf1](const Float stiffness_stretch)
        {
            const Uint vid         = dispatch_id().x;
            ...
            $for(j, num_adj)
            {
                ...
                He = stiffness_stretch_spring * ddT
                     + stiffness_stretch_spring * max(1.0f - L * x_inv, 0.0f) * (make_float3x3(1.0f) - ddT);
...
void DescentSolver::physics_step_GPU(luisa::compute::Device& device, luisa::compute::Stream& stream)
{
    ...
    for (uint substep = 0; substep < num_substep; substep++)
    {
        ...
        {
            stream << fn_evaluate_inertia(substep_dt).dispatch(mesh_data->num_verts)
        ...
        stream << fn_update_velocity(lcs::get_scene_params().get_substep_dt(),
                                     false,
                                     lcs::get_scene_params().damping_cloth)
        ...
        CpuParallel::parallel_for(0,
                                  ...
                                  [&](const uint vid)
                                  {
                                      ...
                                      for (uint j = 0; j < num_adj; j++)
                                      {
                                          ...
                                          float    x_inv         = 1.f / l;
                                          float    x_squared_inv = x_inv * x_inv;
                                          He = stiffness_stretch_spring * x_squared_inv * xxT
                                               + stiffness_stretch_spring * max_scalar(1 - L * x_inv, 0.0f)
                                                     * (luisa::make_float3x3(1.0f) - x_squared_inv * xxT);

                                          const uint offset = vid == edge[0] ? 0 :
                                                              vid == edge[1] ? 1 :
                                                                               -1u;
