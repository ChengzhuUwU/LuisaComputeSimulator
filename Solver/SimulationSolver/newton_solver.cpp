...
void NewtonSolver::host_evaluate_inertia()
{
    ...
    CpuParallel::parallel_for(0, host_mesh_data->num_verts,
        [sa_cgB = host_sim_data->sa_cgB.data(),
            ...
            substep_dt = get_scene_params().get_substep_dt(), stiffness_dirichlet](const uint vid) {
            const float h = substep_dt;
            ...
            }
            {
                if constexpr (print_detail)
                    LUISA_INFO("vid {}, mass: {}, move = {}, gradient: {}, hessian: {}", vid, mass, length_vec(x_k - x_tilde), gradient, hessian);
                // sa_cgX[vid] = dx_0;
                sa_cgB[vid] = gradient;
...
void NewtonSolver::host_evaluate_dirichlet()
{
    ...
    CpuParallel::parallel_for(0, host_mesh_data->num_verts,
        [sa_cgB = host_sim_data->sa_cgB.data(),
            ...
            sa_x_tilde = host_sim_data->sa_x_tilde.data(),
            sa_is_fixed = host_mesh_data->sa_is_fixed.data(),
            sa_vert_mass = host_mesh_data->sa_vert_mass.data(), stiffness_dirichlet, substep_dt](const uint vid) {
...
void NewtonSolver::host_evaluate_ground_collision()
{
    ...
    CpuParallel::parallel_for(0, num_verts,
        [sa_cgB = host_sim_data->sa_cgB.data(),
            ...
            stiffness_ground = 1e7f](const uint vid) {
            if (sa_is_fixed[vid])
                return;
            if (get_scene_params().use_floor) {
                float3 x_k = sa_x[vid];
...
void NewtonSolver::host_evaluete_spring()
{
    ...
    for (uint cluster_idx = 0; cluster_idx < host_sim_data->num_clusters_springs; cluster_idx++) {
        ...
        CpuParallel::parallel_for(0, num_elements_clustered, [sa_x = host_sim_data->sa_x.data(), sa_edges = host_sim_data->sa_merged_stretch_springs.data(), sa_rest_length = host_sim_data->sa_merged_stretch_spring_rest_length.data(), sa_cgB = host_sim_data->sa_cgB.data(), sa_cgA_diag = host_sim_data->sa_cgA_diag.data(), sa_cgA_offdiag_stretch_spring = host_sim_data->sa_cgA_offdiag_stretch_spring.data(), cluster = host_sim_data->sa_clusterd_springs.data() + host_sim_data->num_clusters_springs + 1, curr_prefix, stiffness_stretch = get_scene_params().stiffness_spring](const uint index) {
...
void NewtonSolver::host_evaluete_bending()
{
    ...
    for (uint cluster_idx = 0; cluster_idx < host_sim_data->num_clusters_bending_edges; cluster_idx++) {
        ...
        CpuParallel::parallel_for(0, num_elements_clustered, [sa_x = host_sim_data->sa_x.data(), sa_bending_edges = host_sim_data->sa_merged_bending_edges.data(), sa_bending_edges_Q = host_sim_data->sa_merged_bending_edges_Q.data(), sa_cgB = host_sim_data->sa_cgB.data(), sa_cgA_diag = host_sim_data->sa_cgA_diag.data(), sa_cgA_offdiag_bending = host_sim_data->sa_cgA_offdiag_bending.data(), curr_prefix, stiffness_bending = get_scene_params().get_stiffness_quadratic_bending()](const uint index) {
            // const uint eid = culster[curr_prefix + index];
            const uint eid = curr_prefix + index;
...
void NewtonSolver::host_solve_amgcl(luisa::compute::Stream& stream, std::function<double(const std::vector<float3>&)> func_compute_energy)
{
    auto assmble_amgcl_system = [&](
        ...
        std::vector<std::vector<uint>> adjacency(host_sim_data->vert_adj_material_force_verts);
        {
            ...
            CpuParallel::parallel_for_and_scan<uint>(0, num_verts, [&](const uint vid) { return adjacency[vid].size(); }, [&](const uint vid, const uint block_prefix, const uint parallel_result) { ptr[vid + 1] = block_prefix; }, 0);
...

        {
            ...
            CpuParallel::single_thread_for(0, num_vf + num_ee,
                [narrow_phase_list_vf = host_collision_data->narrow_phase_list_vf.data(),
                    ...
                    ptr = ptr.data(),
                    adjacency = adjacency.data(),
                    val = val.data(), num_vf, num_ee](const uint pair_idx) {
                    uint4 indices;
                    float4 weight;
                    float3 normal;
                    float k2;
...
void NewtonSolver::physics_step_CPU(luisa::compute::Device& device, luisa::compute::Stream& stream)
{
    ...
    // Solve
    auto gradient_descent_solve = [&]() {
        // Actually is Jacobi Preconditioned Gradient-Descent (Without weighting)
        CpuParallel::parallel_for(0, mesh_data->num_verts, [&](const uint vid) {
            float3x3 hessian = host_sim_data->sa_cgA_diag[vid];
            float3 f = host_sim_data->sa_cgB[vid];
