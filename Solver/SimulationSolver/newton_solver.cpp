...
namespace lcs
{
	...
	void NewtonSolver::compile_assembly(AsyncCompiler& compiler, const luisa::compute::ShaderOption& default_option)
	{
		...
		auto add_hess_diag_atomic = [](const auto& sa_cgA_diag, const UInt vid, const Float3x3& H)
		{
			sa_cgA_diag->atomic(vid)[0][0].fetch_add(H[0][0]);
			sa_cgA_diag->atomic(vid)[0][1].fetch_add(H[0][1]);
			sa_cgA_diag->atomic(vid)[0][2].fetch_add(H[0][2]);
			sa_cgA_diag->atomic(vid)[1][0].fetch_add(H[1][0]);
			sa_cgA_diag->atomic(vid)[1][1].fetch_add(H[1][1]);
			sa_cgA_diag->atomic(vid)[1][2].fetch_add(H[1][2]);
			sa_cgA_diag->atomic(vid)[2][0].fetch_add(H[2][0]);
			sa_cgA_diag->atomic(vid)[2][1].fetch_add(H[2][1]);
			sa_cgA_diag->atomic(vid)[2][2].fetch_add(H[2][2]);
		};

		auto assemble_joint = [sa_cgB = sim_data->sa_cgB.view(),
								  sa_cgA_diag = sim_data->sa_cgA_diag.view(),
								  sa_cgA_offdiag_triplet = sim_data->sa_cgA_fixtopo_offdiag_triplet.view(),
								  add_hess_diag_atomic](
								  const UInt  jid,
								  const UInt4 idx_a,
								  const UInt4 idx_b,
								  const Float3(&grad)[8],
								  const Float3x3(&hess)[64],
								  const Var<BufferView<uint>>& offsets)
		{
			UInt nodes[8] = { idx_a.x, idx_a.y, idx_a.z, idx_a.w, idx_b.x, idx_b.y, idx_b.z, idx_b.w };
			for (uint ii = 0; ii < 8; ++ii)
			{
				const UInt	 vid = nodes[ii];
				const Float3 g = grad[ii];
				sa_cgB->atomic(vid)[0].fetch_add(-g[0]);
				sa_cgB->atomic(vid)[1].fetch_add(-g[1]);
				sa_cgB->atomic(vid)[2].fetch_add(-g[2]);
				add_hess_diag_atomic(sa_cgA_diag, vid, hess[ii * 8 + ii]);
				uint off_slot = 0u;
				for (uint jj = 0; jj < 8; ++jj)
				{
					...
					const UInt triplet_idx = offsets->read(jid * 56u + ii * 7u + off_slot);
					off_slot += 1u;
					atomic_add_triplet_matrix(sa_cgA_offdiag_triplet, triplet_idx, hess[ii * 8 + jj]);
				}
			}
		};

		auto eval_and_assemble_fixed = [assemble_joint](const UInt jid,
										   const UInt4			   idx_a,
										   const UInt4			   idx_b,
										   const Float3(&q)[8],
										   const Float3					anchor_a,
										   const Float3					anchor_b,
										   const Float2					stiff,
										   const Var<BufferView<uint>>& offsets)
		{
			Float3	 grad[8];
			...
			for (uint i = 0; i < 8; ++i)
			{
				grad[i] = make_float3(0.0f);
				for (uint j = 0; j < 8; ++j)
				{
					hess[i * 8 + j] = make_float3x3(0.0f);
				}
			}
			const Float3x3 I = make_float3x3(1.0f);
			auto		   add_term = [&](const Float3x3(&coeff)[8], const Float s)
			{
				...
				for (uint i = 0; i < 8; ++i)
				{
					...
					for (uint j = 0; j < 8; ++j)
					{
						hess[i * 8 + j] += s * (coeff[i] * coeff[j]);
					}
				}
			};
			{
				Float3x3 coeff[8] = { (-1.0f) * I, -anchor_a.x * I, -anchor_a.y * I, -anchor_a.z * I,
					I, anchor_b.x * I, anchor_b.y * I, anchor_b.z * I };
				add_term(coeff, stiff.x);
			}
			for (uint row = 0; row < 3; ++row)
			{
				Float3x3 coeff[8] = { make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f),
					make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f) };
				coeff[1 + row] = I;
				coeff[5 + row] = (-1.0f) * I;
				add_term(coeff, stiff.y);
			}
			assemble_joint(jid, idx_a, idx_b, grad, hess, offsets);
		};

		auto eval_and_assemble_prismatic = [assemble_joint](const UInt jid,
											   const UInt4			   idx_a,
											   const UInt4			   idx_b,
											   const Float3(&q)[8],
											   const Float3					anchor_a,
											   ...
											   const Var<BufferView<uint>>& offsets)
		{
			...
			const Float3x3 I = make_float3x3(1.0f);
			const Float3x3 P = I - make_float3x3(axis * axis.x, axis * axis.y, axis * axis.z);
			auto		   add_term = [&](const Float3x3(&coeff)[8], const Float s)
			{
				Float3 r = make_float3(0.0f);
				for (uint i = 0; i < 8; ++i)
			...
			{
				Float3x3 coeff[8] = { (-1.0f) * P, -anchor_a.x * P, -anchor_a.y * P, -anchor_a.z * P,
					P, anchor_b.x * P, anchor_b.y * P, anchor_b.z * P };
				add_term(coeff, stiff.x);
			}
			for (uint row = 0; row < 3; ++row)
			{
		...
		auto eval_and_assemble_revolute = [assemble_joint](const UInt jid,
											  const UInt4			  idx_a,
											  const UInt4			  idx_b,
											  const Float3(&q)[8],
											  const Float3				   anchor_a,
											  ...
											  const Var<BufferView<uint>>& offsets)
		{
			...
			{
				Float3x3 coeff[8] = { make_float3x3(0.0f), axis_a.x * P, axis_a.y * P, axis_a.z * P,
					make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f) };
				add_term(coeff, stiff.y);
			}
			{
				Float3x3 coeff[8] = { make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f), make_float3x3(0.0f),
					make_float3x3(0.0f), axis_b.x * P, axis_b.y * P, axis_b.z * P };
				add_term(coeff, stiff.y);
			}
			assemble_joint(jid, idx_a, idx_b, grad, hess, offsets);
		};
		...
		compiler.compile(
			...
			[sa_q = sim_data->sa_q.view(), eval_and_assemble_fixed](Var<BufferView<uint4>> indices_a,
				...
				Var<BufferView<uint>>													   offsets)
			{
				const UInt	jid = dispatch_id().x;
				const UInt4 idx_a = indices_a->read(jid);
				const UInt4 idx_b = indices_b->read(jid);
				Float3		q[8] = {
				...
				eval_and_assemble_fixed(jid, idx_a, idx_b, q, anchor_a_local->read(jid), anchor_b_local->read(jid), stiffness->read(jid), offsets);
			},
			default_option);

		compiler.compile(
			fn_material_energy_assembly_prismatic_joint,
			[sa_q = sim_data->sa_q.view(), eval_and_assemble_prismatic](Var<BufferView<uint4>> indices_a,
				Var<BufferView<uint4>>														   indices_b,
				Var<BufferView<float3>>														   anchor_a_local,
				Var<BufferView<float3>>														   anchor_b_local,
				...
				Var<BufferView<uint>>														   offsets)
			{
				...
				eval_and_assemble_prismatic(jid, idx_a, idx_b, q, anchor_a_local->read(jid), anchor_b_local->read(jid), axis_world->read(jid), stiffness->read(jid), offsets);
			},
			default_option);

		compiler.compile(
			fn_material_energy_assembly_revolute_joint,
			[sa_q = sim_data->sa_q.view(), eval_and_assemble_revolute](Var<BufferView<uint4>> indices_a,
				Var<BufferView<uint4>>														  indices_b,
				Var<BufferView<float3>>														  anchor_a_local,
				Var<BufferView<float3>>														  anchor_b_local,
				Var<BufferView<float3>>														  axis_world,
				...
				Var<BufferView<uint>>														  offsets)
			{
				...
				eval_and_assemble_revolute(jid, idx_a, idx_b, q, anchor_a_local->read(jid), anchor_b_local->read(jid), axis_world->read(jid), axis_a_local->read(jid), axis_b_local->read(jid), stiffness->read(jid), offsets);
			},
			default_option);
	}
	...
	void NewtonSolver::host_material_energy_assembly()
	{
		// Assemble material forces and stiffness matrix
		{
			...
			auto host_assemble_joint = [&](const uint					   jid,
										   const uint4					   idx_a,
										   const uint4					   idx_b,
										   const std::array<float3, 8>&	   grad,
										   const std::array<float3x3, 64>& hess,
										   const std::span<const uint>	   offsets)
			{
				const uint nodes[8] = { idx_a.x, idx_a.y, idx_a.z, idx_a.w, idx_b.x, idx_b.y, idx_b.z, idx_b.w };
				for (uint ii = 0; ii < 8; ++ii)
				{
					const uint vid = nodes[ii];
					sa_cgB[vid] -= grad[ii];
					const auto H = hess[ii * 8 + ii];
					sa_cgA_diag[vid][0][0] += H[0][0];
					sa_cgA_diag[vid][0][1] += H[0][1];
					sa_cgA_diag[vid][0][2] += H[0][2];
					sa_cgA_diag[vid][1][0] += H[1][0];
					sa_cgA_diag[vid][1][1] += H[1][1];
					sa_cgA_diag[vid][1][2] += H[1][2];
					sa_cgA_diag[vid][2][0] += H[2][0];
					sa_cgA_diag[vid][2][1] += H[2][1];
					sa_cgA_diag[vid][2][2] += H[2][2];
					uint off_slot = 0u;
					for (uint jj = 0; jj < 8; ++jj)
					{
						if (ii == jj)
						...
						const uint triplet_idx = offsets[jid * 56u + ii * 7u + off_slot];
						off_slot += 1u;
						auto& triplet = sa_cgA_offdiag_triplet[triplet_idx];
						add_triplet_matrix(triplet, hess[ii * 8 + jj]);
					}
				}
			};

			const float3x3 I = luisa::make_float3x3(1.0f);
			auto&		   fixed_joint = host_sim_data->get_fixed_joint_data();
			if (fixed_joint.is_valid())
			{
				auto indices_a = std::span(fixed_joint.constraint_indices_a);
				auto indices_b = std::span(fixed_joint.constraint_indices_b);
				auto anchor_a = std::span(fixed_joint.anchor_a_local);
				auto anchor_b = std::span(fixed_joint.anchor_b_local);
				auto stiffness = std::span(fixed_joint.stiffness);
				auto offsets = std::span(fixed_joint.constraint_offsets_in_adjlist);
				auto q = std::span(host_sim_data->sa_q);
				for (uint jid = 0; jid < fixed_joint.get_num_indices(); ++jid)
				{
					const auto ia = indices_a[jid];
					const auto ib = indices_b[jid];
					float3	   q_joint[8] = { q[ia.x], q[ia.y], q[ia.z], q[ia.w], q[ib.x], q[ib.y], q[ib.z], q[ib.w] };
					auto	   eval = detail::fixed_joint_constaint::evaluate<float, float3, float3x3>(
						  ...
						  stiffness[jid].y,
						  I);
					host_assemble_joint(jid, ia, ib, eval.gradients, eval.hessians, offsets);
				}
			}

			auto& prismatic_joint = host_sim_data->get_prismatic_joint_data();
			if (prismatic_joint.is_valid())
			{
				auto indices_a = std::span(prismatic_joint.constraint_indices_a);
				auto indices_b = std::span(prismatic_joint.constraint_indices_b);
				auto anchor_a = std::span(prismatic_joint.anchor_a_local);
				auto anchor_b = std::span(prismatic_joint.anchor_b_local);
				auto axis_world = std::span(prismatic_joint.axis_world);
				auto stiffness = std::span(prismatic_joint.stiffness);
				auto offsets = std::span(prismatic_joint.constraint_offsets_in_adjlist);
				auto q = std::span(host_sim_data->sa_q);
				for (uint jid = 0; jid < prismatic_joint.get_num_indices(); ++jid)
				{
					const auto ia = indices_a[jid];
					const auto ib = indices_b[jid];
					float3	   q_joint[8] = { q[ia.x], q[ia.y], q[ia.z], q[ia.w], q[ib.x], q[ib.y], q[ib.z], q[ib.w] };
			...
			auto& revolute_joint = host_sim_data->get_revolute_joint_data();
			if (revolute_joint.is_valid())
			{
				auto indices_a = std::span(revolute_joint.constraint_indices_a);
				auto indices_b = std::span(revolute_joint.constraint_indices_b);
				auto anchor_a = std::span(revolute_joint.anchor_a_local);
				auto anchor_b = std::span(revolute_joint.anchor_b_local);
				auto axis_world = std::span(revolute_joint.axis_world);
				auto axis_a = std::span(revolute_joint.axis_a_local);
				auto axis_b = std::span(revolute_joint.axis_b_local);
				auto stiffness = std::span(revolute_joint.stiffness);
				auto offsets = std::span(revolute_joint.constraint_offsets_in_adjlist);
				auto q = std::span(host_sim_data->sa_q);
				for (uint jid = 0; jid < revolute_joint.get_num_indices(); ++jid)
				{
					const auto ia = indices_a[jid];
					const auto ib = indices_b[jid];
					float3	   q_joint[8] = { q[ia.x], q[ia.y], q[ia.z], q[ia.w], q[ib.x], q[ib.y], q[ib.z], q[ib.w] };
		...
		// for (uint substep = 0; substep < get_scene_params().num_substep; substep++)
		{
			...
			for (iter = 0; iter < 100; iter++)
			{
...

				{
					...
					{
						stream << fn_material_energy_assembly_fixed_joint(fixed_joint_data.constraint_indices_a,
							...
							fixed_joint_data.constraint_offsets_in_adjlist)
									  .dispatch(fixed_joint_data.get_num_indices());
					}

					const auto& prismatic_joint_data = sim_data->get_prismatic_joint_data();
					if (prismatic_joint_data.is_valid())
					{
						stream << fn_material_energy_assembly_prismatic_joint(prismatic_joint_data.constraint_indices_a,
							...
							prismatic_joint_data.constraint_offsets_in_adjlist)
									  .dispatch(prismatic_joint_data.get_num_indices());
					}

					const auto& revolute_joint_data = sim_data->get_revolute_joint_data();
					if (revolute_joint_data.is_valid())
					{
						stream << fn_material_energy_assembly_revolute_joint(revolute_joint_data.constraint_indices_a,
							...
							revolute_joint_data.constraint_offsets_in_adjlist)
									  .dispatch(revolute_joint_data.get_num_indices());
					}
