...
inline T1 max_scalar(const T1 &left, const T2 &right) { return left > right ? left : right; }
template<typename T1, typename T2>
inline T1 min_scalar(const T1 &left, const T2 &right) { return left < right ? left : right; }

template<typename T>
static inline bool default_compate(const T &left, const T &right) { return left < right; }
...
void parallel_for(uint start_pos, uint end_pos, FuncName func, const uint blockDim = 256) {
    ...
    tbb::parallel_for(tbb::blocked_range<uint>(start_dispatch, end_dispatch, 1), [&](tbb::blocked_range<uint> r) { 
...
void parallel_for_each_core(uint start_core_idx, uint end_core_idx, FuncName func) {
    ...
    tbb::parallel_for(tbb::blocked_range<uint>(start_core_idx, end_core_idx, 1), [&](tbb::blocked_range<uint> r) { 
            uint blockIdx = r.begin();
            func(blockIdx); }, tbb::simple_partitioner{});
}

template<typename T, typename ParallelFunc, typename ReduceFuncBinary>
inline T parallel_for_and_reduce(uint start_pos, uint end_pos, ParallelFunc func_parallel, ReduceFuncBinary func_binary, const T zero, const uint blockDim = 256) {
    uint start_dispatch = start_pos / blockDim;
    uint end_dispatch = (end_pos + blockDim - 1) / blockDim;
    // parallel_reduce
    return tbb::parallel_deterministic_reduce(tbb::blocked_range<uint>(start_dispatch, end_dispatch, 1), zero, [&](tbb::blocked_range<uint> r, T result) {
            uint blockIdx = r.begin();
            uint startIdx = max_scalar(blockDim * blockIdx, start_pos);
            uint endIdx = min_scalar(blockDim * (blockIdx + 1), end_pos);
            ...
            return result; }, func_binary, tbb::simple_partitioner{});
}
...
inline void parallel_for_and_scan(uint start_pos, uint end_pos, ParallelFunc func_parallel, OutputFunc func_output, const T &zero) {
    ...
    tbb::parallel_scan(tbb::blocked_range<uint>(start_dispatch, end_dispatch, 1), zero, [&](tbb::blocked_range<uint> r, T block_prefix, auto is_final_scan) -> T {
            ...
            return block_prefix; }, [](const T &x, const T &y) -> T { return x + y; }, tbb::simple_partitioner{});
...
template<typename FuncName>
void parallel_for(uint start_pos, uint end_pos, FuncName func, const uint blockDim = 256) {
    luisa::fiber::parallel(start_pos, end_pos, blockDim, [&](auto b, auto e) {
        for (auto i = b; i < e; ++i) {
            func(i);
        }
    });
...
template<typename T, typename ParallelFunc, typename OutputFunc>
inline void parallel_for_and_scan(uint start_pos, uint end_pos, ParallelFunc func_parallel, OutputFunc func_output, const T &zero, const uint blockDim = 256) {
    ...
    std::inclusive_scan(thread_result.begin(), thread_result.end(), thread_result.begin(), [](const T &left, const T &right) { return left + right; }, zero);
...
inline T parallel_for_and_reduce_sum(uint start_pos, uint end_pos, ParallelFunc func_parallel) {
    return parallel_for_and_reduce<T>(start_pos, end_pos, func_parallel,
