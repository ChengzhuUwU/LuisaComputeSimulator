...
void parallel_for_each_core(uint start_core_idx, uint end_core_idx, FuncName func)
{
    luisa::fiber::parallel(start_core_idx, end_core_idx, 1, [&](auto b, auto e) { func(b); });
}
                           ...
                           [&](auto b, auto e)
                           {
                               for (auto index = b; index < e; index++)
                               {
                                   func_output(index,
                                               ...
                                               index == start_pos ? thread_result.front() :
                                                                    thread_result[index - start_pos]
                                                                        - thread_result[index - start_pos - 1]);
                               }
                           });
}
...
inline void parallel_for_and_scan(uint start_pos, uint end_pos, ParallelFunc func_parallel, OutputFunc func_output, const T& zero)
{
    ...
    for (uint index = start_pos; index < end_pos; index++)
    {
        func_output(index,
                    thread_result[index - start_pos],
                    index == start_pos ? thread_result.front() :
                                         thread_result[index - start_pos] - thread_result[index - start_pos - 1]);
    }
}
...
inline T single_thread_for_and_reduce_sum(uint start_pos, uint end_pos, ParallelFunc func_parallel)
{
    ...
    return std::reduce(
        thread_values.begin(), thread_values.end(), T(), [](const T& x, const T& y) -> T { return x + y; });
}
...
{
    parallel_for(0, array_size, [&](const uint index) { dst[index] = src[index]; });
}
...
{
    parallel_for(0, array_size, [&](const uint index) { dst[index] = value; });
}
template <typename T>
